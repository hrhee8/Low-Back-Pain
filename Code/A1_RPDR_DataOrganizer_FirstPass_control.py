...
#%% (2.1) Find all the columns with all existing inclusion code
...
project_icd_codes3 = util_rpdr.get_icds_fromfile(path_inclusion_crit3)
project_icd_codes4 = util_rpdr.get_icds_fromfile(path_inclusion_crit4)
project_icd_codes5 = util_rpdr.get_icds_fromfile(path_inclusion_crit5)
 
diag_icd10_criteria_LBP = diagnoses[((diagnoses['Code_Type'] == "ICD10") | (diagnoses['Code_Type'] == "ICD9")) &
                                     ((diagnoses["Code"].str.contains(project_icd_codes, regex=True)) | (diagnoses["Code"].str.contains(project_icd_codes2, regex=True))
                                     | (diagnoses["Code"].str.contains(project_icd_codes3, regex=True)) | (diagnoses["Code"].str.contains(project_icd_codes4, regex=True))
                                     | (diagnoses["Code"].str.contains(project_icd_codes5, regex=True)))]
#%% (2.2) exclude the inclusion codes
diag_icd10_criteria = diagnoses.loc[~((diagnoses.EMPI.isin(diag_icd10_criteria_LBP['EMPI']))),:]
#%% (2.3) save in h5 file
print(diag_icd10_criteria)
with pd.HDFStore(h5_filepath) as store:
...
#%% (3) Merge the tables from EMPIS
print(proc_cpt_incl.shape)
